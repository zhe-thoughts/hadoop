...
public class TestAddStripedBlocks {
  ...
  public void testAddStripedBlock() throws Exception {
    ...
    try {
      ...
      checkStripedBlockUC(fileNode.getLastBlock(), true);
      ...
      assertEquals(1, blocks.length);
      Assert.assertTrue(blocks[0].isStriped());
      checkStripedBlockUC(fileNode.getLastBlock(), false);
  ...
  private void checkStripedBlockUC(BlockInfo block,
      boolean checkReplica) {
    assertEquals(0, block.numNodes());
    Assert.assertFalse(block.isComplete());
    Assert.assertEquals(HdfsConstants.NUM_DATA_BLOCKS, block.getStripedBlockStorageOp().getDataBlockNum());
    Assert.assertEquals(HdfsConstants.NUM_PARITY_BLOCKS,
        block.getStripedBlockStorageOp().getParityBlockNum());
  ...
  public void testCheckStripedReplicaCorrupt() throws Exception {
    ...
    BlockInfo stored = fileNode.getStripedBlocksFeature().getBlocks()[0];
